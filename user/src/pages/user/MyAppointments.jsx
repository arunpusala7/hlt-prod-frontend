...
function MyAppointments() {
  const { tenant, isTenantActive } = useTenant();
  ...
  const getApptClinic = (appt) => {
    if (!appt) return null;
    if (appt.clinic && typeof appt.clinic === "object") return appt.clinic;
    if (appt.clinicId && tenant?.clinics?.length) {
      const found = tenant.clinics.find(
        (c) => String(c.id) === String(appt.clinicId) || String(c._id) === String(appt.clinicId)
      );
      if (found) return found;
    }
    if (appt.clinicName && tenant?.clinics?.length) {
      const found = tenant.clinics.find(
        (c) => c.name?.trim().toLowerCase() === appt.clinicName?.trim().toLowerCase()
      );
      if (found) return found;
    }
    return null;
  ...
  const isApptClinicPaused = (appt) => {
    if (isTenantActive === false) return true;
    const clinic = getApptClinic(appt);
    if (clinic) {
      return !isClinicActive(clinic, tenant);
    }
    return false;
  };
  ...
  const openRescheduleModal = (appt) => {
    if (isTenantActive === false) {
      toast.error("Rescheduling is temporarily unavailable: Hospital network services are paused.");
      return;
    }
    if (isApptClinicPaused(appt)) {
      toast.error("Rescheduling is unavailable: This clinic branch is currently paused.");
      return;
    }
  ...
  const handleConfirmReschedule = async () => {
    ...
    if (isApptClinicPaused(selectedAppt)) {
      toast.error("Rescheduling is unavailable: This clinic branch is currently paused.");
      return;
    }
          ...
          >
            <div>
                ...
                <div style={styles.docRow}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap", marginTop: "2px" }}>
                      ...
                      {isApptClinicPaused(a) && (
                        <span style={styles.pausedApptBadge} title="This clinic branch is temporarily inactive. Past records and passes remain fully accessible.">
                          ● Branch Paused
                        </span>
                      )}
                    </div>
              ...
              <div style={styles.detailsBox}>
                ...
                {(a.clinicName || getApptClinic(a)?.name) && (
                  <div style={styles.detailItem}>
                    <span style={styles.detailLabel}>Branch</span>
                    <span style={{ ...styles.detailVal, color: isApptClinicPaused(a) ? "#DC2626" : "#0F172A" }}>
                      {a.clinicName || getApptClinic(a)?.name}
                    </span>
                  </div>
                )}
            ...
            <div style={styles.actionsRow}>
              {(a.status === "BOOKED" || a.status === "RESCHEDULED") && (
                <>
                  ...
                  <motion.button 
                    whileTap={{ scale: isApptClinicPaused(a) ? 1 : 0.96 }}
                    style={isApptClinicPaused(a) ? styles.btnRescheduleDisabled : styles.btnReschedule} 
                    onClick={() => openRescheduleModal(a)}
                    title={isApptClinicPaused(a) ? "Rescheduling is unavailable for paused clinic branches" : "Reschedule consultation"}
                  >
                    Reschedule ↻
                  </motion.button>
                </>
        ...
        <AnimatePresence>
          {rescheduleModalOpen && (
            ...
            >
              ...
              <div style={styles.sheetBody}>
                {isApptClinicPaused(selectedAppt) ? (
                  <div style={styles.branchPausedBanner}>
                    <span style={{ fontSize: "20px", flexShrink: 0 }}>⚠️</span>
                    <div>
                      <h4 style={styles.branchPausedTitle}>Branch Scheduling Paused</h4>
                      <p style={styles.branchPausedDesc}>
                        This clinic branch is currently inactive. Rescheduling and new appointments cannot be booked at this location.
                      </p>
                    </div>
                  </div>
                ) : (
              ...
              <div style={styles.sheetStickyBottom}>
                ...
                <button 
                  ...
                  disabled={!newSlot || isApptClinicPaused(selectedAppt)} 
                  style={newSlot && !isApptClinicPaused(selectedAppt) ? styles.modalConfirmPill : styles.modalDisabledPill}
                >
                  Confirm Reschedule
                </button>
              </div>
