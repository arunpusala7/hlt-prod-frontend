...
function UserOverview({ 
  ...
  const handleOpenReschedule = (appt) => {
    ...
    const doc = safeDoctors.find(d => String(d.id) === String(docId));
    if (doc && !isDoctorActiveForBooking(doc, tenant)) {
      toast.error(`Cannot reschedule: ${getBookingDisabledReason(doc, tenant)}`);
      return;
    }
  ...
  return (
    <div style={isPhoneMode ? styles.phoneContainer : styles.fluidContainer}>
              ...
              {outlets.map((outlet) => {
                ...
                return (
                  <motion.button
                    ...
                    style={isActive ? styles.specChipActive : (isOutletActive ? styles.specChipInactive : styles.specChipPaused)}
                    title={outlet.fullName ? `${outlet.fullName} (${count} doctors)${!isOutletActive ? " - Branch Paused" : ""}` : undefined}
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={!isOutletActive ? "#DC2626" : "currentColor"} strokeWidth="2.2">
                    ...
                    {!isOutletActive && (
                      <span style={styles.outletPausedBadge}>Paused</span>
                    )}
      ...
      <div className="merged-doctor-section" style={styles.mergedDoctorSection}>
        {/* Branch Paused Banner when an inactive outlet is selected */}
        {(() => {
          const selOutletObj = isOrganization && selectedOutlet ? outlets.find(o => String(o.id) === String(selectedOutlet)) : null;
          if (selOutletObj && selOutletObj.active === false) {
            return (
              <div style={styles.branchPausedBanner}>
                <div style={styles.branchPausedIcon}>⚠️</div>
                <div style={{ flex: 1 }}>
                  <h4 style={styles.branchPausedTitle}>Branch Temporarily Paused: {selOutletObj.name}</h4>
                  <p style={styles.branchPausedDesc}>
                    Appointment bookings are currently suspended for this branch location. Specialists at this clinic cannot take patient slots while the clinic is paused.
                  </p>
                </div>
              </div>
            );
          ...
          {loadingDoctors ? (
            ...
            filteredDoctors.map((doc) => {
              ...
              <motion.div
                ...
                whileTap={isDocBookable ? { scale: 0.968 } : undefined}
                className="doc-card tactile-card"
                style={{
                  ...styles.docCard,
                  ...(isDocBookable ? {} : styles.docCardPaused)
                }}
                onClick={() => {
                  if (!isDocBookable) {
                    toast.error(getBookingDisabledReason(doc, tenant));
              ...
              >
                <div style={styles.docCardAvatar}>
                  <img 
                    ...
                    style={{
                      ...styles.docCardImg,
                      ...(isDocBookable ? {} : { filter: "grayscale(30%)", opacity: 0.85 })
                    }} 
                  />
                  {!isDocBookable && (
                ...
                <div style={styles.docCardBody}>
                  ...
                  {(doc.clinic?.branchName || doc.clinicName || doc.hospitalAffiliation) && (
                    <div style={{ display: "flex", alignItems: "center", gap: "4px", marginTop: "3px", fontSize: "11.5px", color: "#64748B" }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={!isDocBookable ? "#DC2626" : "#3B82F6"} strokeWidth="2.5">
                        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                        <circle cx="12" cy="10" r="3"></circle>
                      </svg>
                  ...
                  <button
                    style={isDocBookable ? styles.docCardBtn : styles.docCardBtnDisabled}
                  ...
                  >
                    {isDocBookable ? "Book Appointment" : "Branch Paused"}
                  </button>
                </div>
              </motion.div>
              );
          ...
          {bookingDoctor && (
            ...
            >
              ...
              <div style={styles.sheetScrollBody}>
                ...
                {!isDoctorActiveForBooking(bookingDoctor, tenant) && (
                  <div style={styles.branchPausedBanner}>
                    <div style={styles.branchPausedIcon}>⚠️</div>
                    <div style={{ flex: 1 }}>
                      <h4 style={styles.branchPausedTitle}>Appointment Booking Paused</h4>
                      <p style={styles.branchPausedDesc}>
                        {getBookingDisabledReason(bookingDoctor, tenant)} New consultations and waitlists cannot be scheduled while this branch is paused.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div style={styles.sheetDateStrip}>
                  ...
                  <div
                    onClick={() => {
                      if (!isDoctorActiveForBooking(bookingDoctor, tenant)) {
                        toast.error(getBookingDisabledReason(bookingDoctor, tenant));
                        return;
                      }
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          handleBookingDateChange(item.isoDate);
                        }}
                ...
                {!isDoctorActiveForBooking(bookingDoctor, tenant) ? (
                  <div style={styles.emptySlotsBox}>
                    <p style={{ margin: "0 0 6px 0", fontSize: "14px", fontWeight: "700", color: "#B45309" }}>⚠️ Branch Booking Paused</p>
                    <p style={{ margin: "0", fontSize: "12.5px", color: "#92400E", lineHeight: "1.5" }}>
                      {getBookingDisabledReason(bookingDoctor, tenant)}
                    </p>
                  </div>
                ) : loadingBookingSlots ? (
                ...
                {/* Highlight Boxes: Success Stories & Testimonials */}
                {(bookingDoctor.successStories || bookingDoctor.testimonials) && (
                  <div style={styles.sheetStoriesBox}>
              ...
              <div style={styles.sheetBottomBar}>
                {(() => {
                  const isBookingDocActive = isDoctorActiveForBooking(bookingDoctor, tenant);
                  return (
                    <button
                      disabled={!isBookingDocActive || !selectedBookingSlot || isProcessingPayment}
                      onClick={handleConfirmBookingAndPay}
                      style={isBookingDocActive && selectedBookingSlot ? styles.sheetBookBtnActive : styles.sheetBookBtnDisabled}
                    >
                      {!isBookingDocActive
          ...
          {reschedulingAppt && (
            ...
            >
              ...
              <div style={styles.sheetBody}>
                ...
                {(() => {
                  const rDocId = reschedulingAppt.doctorId || reschedulingAppt.doctor?.id;
                  const rDoc = safeDoctors.find(d => String(d.id) === String(rDocId));
                  const isReschedAllowed = rDoc ? isDoctorActiveForBooking(rDoc, tenant) : true;
                  if (!isReschedAllowed) {
                    return (
                      <div style={styles.branchPausedBanner}>
                        <div style={styles.branchPausedIcon}>⚠️</div>
                        <div style={{ flex: 1 }}>
                          <h4 style={styles.branchPausedTitle}>Rescheduling Unavailable</h4>
                          <p style={styles.branchPausedDesc}>
                            The clinic branch for this consultation is currently inactive. You may keep your confirmed appointment, cancel it, or schedule with an active specialist at another clinic branch.
                          </p>
                        </div>
                      </div>
                    );
              ...
              <div style={styles.sheetStickyBottom}>
                ...
                {(() => {
                  ...
                  return (
                    <button
                      disabled={!isReschedAllowed || !selectedRescheduleSlot || isConfirmingReschedule}
                      onClick={handleConfirmRescheduleSubmit}
                      style={isReschedAllowed && selectedRescheduleSlot ? styles.sheetBookBtnActive : styles.sheetBookBtnDisabled}
                    >
                      {!isReschedAllowed
                        ? "Branch Paused - Cannot Reschedule"
