...
function DoctorProfile() {
  ...
  const handleBookAndPay = async () => {
    if (!isDoctorActiveForBooking(doctor, tenant)) {
      toast.error(getBookingDisabledReason(doctor, tenant));
      return;
    }
  ...
  return (
    <div style={styles.pageCanvas}>
      <div style={styles.mobileFrame}>
        ...
        {!isDoctorActiveForBooking(doctor, tenant) && (
          <div style={styles.branchPausedBanner}>
            ...
            <div>
              ...
              <p style={styles.branchPausedDesc}>
                {getBookingDisabledReason(doctor, tenant)} Consultations and slot reservations cannot be scheduled at this time.
              </p>
            </div>
          </div>
        )}
        ...
        <div style={styles.slotCard}>
          <h3 style={styles.slotHeading}>Available Slots</h3>

          {!isDoctorActiveForBooking(doctor, tenant) ? (
            <div style={{ textAlign: "center", padding: "18px 12px", background: "#FEF2F2", borderRadius: "12px", border: "1px solid #FECACA" }}>
              <p style={{ margin: "0 0 4px 0", fontSize: "13px", fontWeight: "700", color: "#991B1B" }}>⚠️ Booking Paused</p>
              <p style={{ margin: 0, fontSize: "12px", color: "#DC2626" }}>
                {getBookingDisabledReason(doctor, tenant)}
              </p>
            </div>
          ) : loadingSlots ? (
        ...
        <div style={styles.bottomBar}>
          {(() => {
            const isDocBookable = isDoctorActiveForBooking(doctor, tenant);
            return (
              <button
                onClick={handleBookAndPay}
                disabled={!isDocBookable || !selectedSlot || isProcessingPayment}
                style={isDocBookable && selectedSlot && !isProcessingPayment ? styles.bookPillBtn : styles.bookPillBtnDisabled}
              >
                {!isDocBookable
